...
import { API_DISPATCH, UPLOAD_IMG_SINGLE } from '../../../config/urlConfig';
...
function TaskDialog(props) {
  ...
  const [currentFile, setCurrentFile] = React.useState({ name: '', size: 0, originFile: '', type: '', url: '', documentaryId: '', fileName: '' });
  ...
  useEffect(() => {
    ...
    const taskPromise = fetch(`${API_DISPATCH}/${taskId}`, {
      method: 'GET',
      headers: {
    ...
    const projectPromise = fetch(`${API_DISPATCH}/${projectId}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`,
        'Content-Type': 'application/json',
    ...
    const filePromise = fetch(`${API_DISPATCH}/file/${taskId}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`,
        'Content-Type': 'application/json',
  ...
  async function saveFileUpload() {
    try {
      ...
      const dataResponTask = await fetch(`${API_DISPATCH}/file/${taskId}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
