...
function* postDataSaga(action) {
  try {
    ...
    if (message && typeof message === 'string') {
      ...
      arraymess && Array.isArray(arraymess) && arraymess.length ? mess = arraymess[0] : mess = message
    }
    yield put(changeSnackbar({ status: true, message: mess ? mess : 'Thêm mới thất bại', variant: 'error' }));
  }
}
