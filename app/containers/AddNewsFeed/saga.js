...
import { getTemplateSuccess, putTemplateFailed, postTemplateFailed, getAllTemplateSuccess, getAllTemplateFailure, getTemplateFailure, getTemplate, getDefaultState } from './actions';
...
function* getTemplateSaga(action) {
  try {
    ...
    // if (action.id === 'add') yield put(getTemplateSuccess(data))
    yield put(changeSnackbar({ variant: 'error', status: true, message: 'Lấy dữ liệu thất bại' }));
  }
}
